...
const ArtificialPlants = () => {
  ...
  const handleRangeChange = async (newRange) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/artificial_plant/load/by_price?offset=0&limit=12&from=${newMinPrice}&to=${newMaxPrice}`
      );
      if (!response.ok) {
  ...
  const getAllData = async (pageno) => {
    let res = await fetch(
      `http://localhost:8080/artificial_plant/load?offset=${pageno}&limit=12`
    );
