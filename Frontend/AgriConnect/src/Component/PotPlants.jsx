...
const PotPlants = () => {
  ...
  const handleRangeChange = async (newRange) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/pot_planters/load/by_price?offset=0&limit=12&from=${newMinPrice}&to=${newMaxPrice}`
      );
      if (!response.ok) {
  ...
  const handleChangeStock = async (e) => {
    let url = "http://localhost:8080/pot_planters/load";
    if (e.target.checked) {
      console.log("checked");
  ...
  const handleSortChange = async (event) => {
    ...
    if (SortValue === "Alphabetically, A-Z") {
      url = url + `/SORTBYTITLE_A-Z?offset=1&limit=12`;
      console.log(url);
    } else if (SortValue === "Alphabetically, Z-A") {
      url = url + `/SORTBYTITLE_Z-A?offset=1&limit=12`;
      console.log(url);
  ...
  return (
    <div id={style.PlantParent}>
      ...
      <h1 className={style.PlantsHeading}>PotPlants</h1>
          ...
          <div>
            {data.length === 0 ? (
              ...
              <div className={style.PlantContainer}>
                {data.map((ele, ind) => {
                  const images = JSON.parse(ele.ppimages);
                  const Price = JSON.parse(ele.ppprice);
                  return (
                    <div className={style.PlantCard} key={ind}>
                      ...
                      <div className={style.ButtonBox}>
                        <NavLink to={`/potplantsdetails/${ele.ppid}`}>
